/*
  ...
  Copyright (C) 1994, 2004 Matt Porter, Michael D. Black
...
bibi(int n)
{
	...
	canit(STDOUT_FILENO);
	io_mode(0,0);
    DO_SYSLOG((LOG_INFO, "caught signal %d - exiting", n));
...
#  endif
	...
	if(io_mode(0,0) != OK) {
        DO_SYSLOG((LOG_INFO, "Error in io_mode(0,0)"));
    }
    ...
    DO_SYSLOG((LOG_INFO, "Exiting (%d)", exitcode));
	exit(exitcode);
}
	...
	if (protocol!=ZM_XMODEM || argc==0) {
		...
		if ((c=tryz())!=0) {
			...
			for (;;) {
                ...
                if (enable_syslog && protocol==ZM_YMODEM) {
                    lsyslog(LOG_INFO,"Receiving %ld bytes of %s", 
                            zi.bytes_total, shortname);
                }
#endif
				if (wcrx(&zi)==ERROR)
					goto fubar;
				...
				if (Verbose > 1
					...
					if (enable_syslog) {
                        if (zi.modtime) {
                            struct tm tm;
                            char tmbuf[31];
                            strftime(tmbuf, 31, "%Y-%m-%d %H:%M:%S", localtime_r(&(zi.modtime), &tm));
                            lsyslog(LOG_INFO,"Received %ld bytes of %s/%s: %ld Bytes, %ld BPS (%s)",
							        zi.bytes_total, shortname, protname(),
                                    zi.bytes_received, bps, tmbuf);
...
/*         putc('\n', stdout); */
/*         fprintf(stdout, "%s: ready to receive %s", program_name, Pathname); */
/*         fputs('\r\n', stdout); */
/*         fflush(stdout); */
...
fubar:
	...
	if (Restricted && Pathname) {
		// Don't delete partial data files, save as much as you got
#ifdef DEAD
		unlink(Pathname);
		vstringf(_("\r\n%s: %s removed.\r\n"), program_name, Pathname);
#else
		char *system_cmd=malloc(2*PATH_MAX);
        char *datetime=malloc(2*PATH_MAX);
        struct tm tm;
        time_t    secs;
		int       result;
        ...
        secs = time(NULL);
        gmtime_r(&secs, &tm);
        strftime(datetime, 2*PATH_MAX - 1, "%Y%m%d%H%M%S", &tm);
        snprintf(system_cmd, 2*PATH_MAX - 1, "%s.%s", Pathname, datetime);
        result = rename(Pathname, system_cmd);
		DO_SYSLOG((LOG_INFO, "processed partial file %s->%s (0x%x)",Pathname, system_cmd, result));
        
		//if (!system_cmd) error(1,0,_("out of memory"));
		//sprintf(system_cmd,"/usr/bin/perl /usr/local/basestation/lrz_drop.pl %s",Pathname);
		//result=system(system_cmd); // Process it
		//DO_SYSLOG((LOG_INFO, "processed partial file %s (0x%x)",Pathname,result));
		//free(system_cmd);
        //free(datetime);
#endif // DEAD
	}
...
et_tu:
	...
	while ((c = wcgetsec(&Blklen, rpn, 100)) != 0) {
		if (c == WCEOT) {
			...
			DO_SYSLOG((LOG_INFO, "Pathname fetch returned EOT"));
			sendline(ACK);
			flushmo();
			purgeline(0);	/* Do read next time ... */
			READLINE_PF(1);
...
wcrx(struct zm_fileinfo *zi)
{
	...
	for (;;) {
		...
		DO_SYSLOG((LOG_INFO, "sector number = %d, block length = %d", sectcurr, Blklen));
		report(sectcurr);
		if (sectcurr==((sectnum+1) &0377)) {
            ...
            DO_SYSLOG((LOG_INFO, "received dup sector = %d", sectcurr));
			sendchar=ACK;
		}
		else if (sectcurr==WCEOT) {
			if (closeit(zi)) {
                DO_SYSLOG((LOG_INFO, "error closing file - no ACK sent"));
				return ERROR;
			}
            DO_SYSLOG((LOG_INFO, "done - sending ACK"));
			sendline(ACK);
			flushmo();
			purgeline(0);	/* Do read next time ... */
			//sleep(1);     // GBS - Evidence suggested that the ACK was not making it back to the 
                            // glider and this was a way to prevent it being lost at shutdown.  However
                            // on the NULL modem, this causes all sorts of problems and it looks like the 
                            // issue isn't in this process
			return OK;
		}
		else if (sectcurr==ERROR){
            DO_SYSLOG((LOG_INFO, "did not recieve a valid sector"));
			return ERROR;
        } else {
            DO_SYSLOG((LOG_INFO, "sync error in protocol"));
			zperr( _("Sync Error"));
			return ERROR;
		}
	}
...
wcgetsec(size_t *Blklen, char *rxbuf, unsigned int maxtime)
{
	...
	for (Lastrx=errors=0; errors<RETRYMAX; errors++) {
		...
		if (firstch==SOH) {
			*Blklen=128;
get2:
			sectcurr=READLINE_PF(Rxtimeout);
			if ((sectcurr+(oldcrc=READLINE_PF(Rxtimeout)))==0377) {
				oldcrc=checksum=0;
				for (p=rxbuf,wcj=*Blklen; --wcj>=0; ) {
					if ((firstch=READLINE_PF(Rxtimeout)) < 0) {
                        DO_SYSLOG((LOG_INFO, "timeout reading byte %d", *Blklen - wcj));
						goto bilge;
                    }
				...
				if ((firstch=READLINE_PF(Rxtimeout)) < 0) {
                    DO_SYSLOG((LOG_INFO, "timeout reading final byte"));
					goto bilge;
                }
				if (Crcflg) {
					oldcrc=updcrc(firstch, oldcrc);
					if ((firstch=READLINE_PF(Rxtimeout)) < 0) {
                        DO_SYSLOG((LOG_INFO, "timeout reading second crc byte"));
						goto bilge;
                    }
					oldcrc=updcrc(firstch, oldcrc);
					if (oldcrc & 0xFFFF) {
                        DO_SYSLOG((LOG_INFO, "CRC error sector number = %d", sectcurr));
						zperr( _("CRC"));
                    }
					else {
						Firstsec=FALSE;
                    ...
                    DO_SYSLOG((LOG_INFO, "checksum error sector = %d", sectcurr));
					zperr( _("Checksum"));
                }
			}
			else {
                DO_SYSLOG((LOG_INFO, "sector number garbled = %d", sectcurr));
		...
		/* make sure eot really is eot and not just mixmash */
            ...
            DO_SYSLOG((LOG_INFO, "received EOT and read timed out"));
			...
			if (Lastrx==CAN) {
                DO_SYSLOG((LOG_INFO, "sender canceled"));
			...
			if (Firstsec) {
                DO_SYSLOG((LOG_INFO, "timeout trying to read first sector"));
				goto humbug;
            }
bilge:
            DO_SYSLOG((LOG_INFO, "timeout trying to read next sector"));
			zperr( _("TIMEOUT"));
		}
		else {
            DO_SYSLOG((LOG_INFO, "got 0x%x sector header", firstch));
...
humbug:
		Lastrx=0;
		{
			...
			while(cnt-- && READLINE_PF(1)!=TIMEOUT)
				;
            DO_SYSLOG((LOG_INFO, "finished waiting for next line - cnt = %d", cnt));
		}
		if (Firstsec) {
			sendline(Crcflg?WANTCRC:NAK);
			flushmo();
...
procheader(char *name, struct zm_fileinfo *zi)
{
	...
	if (*nameend) {	/* file coming from Unix or DOS system */
		long modtime;
		long bytes_total;
		int mode, scanned, files_left;
		scanned = sscanf(nameend, "%ld %ld %o %*d %d", &bytes_total, &modtime, &mode, &files_left);
		zi->modtime=modtime;
		zi->bytes_total=bytes_total;
		zi->mode=mode;
		if (zi->mode & UNIXFILE)
			++Thisbinary;
        if (scanned == 4 && files_left == 1)
            zi->last_file = 1;
	}
...
#endif
    chmod(Pathname, 00644); // for Seaglider ops
