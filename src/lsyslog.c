/*
  ...
  Copyright (C) 1997, 2004 Uwe Ohse
...
#include "zglobal.h"
#include <pwd.h>
#include <stdio.h> /* Added for logging to file */
#include <stdlib.h>
#include <string.h>
#endif
...
#endif
{
    ...
    /* syslog(prio,"[%s] %s",username,s); */
    {
        ...
        if ((logfp = fopen("comm.log", "a")) != NULL) {
			...
			time(&t); // now
			//ctime_r(&t,timebuf);
			//timebuf[24] = (char)NULL;
            strftime(timebuf, 26, "%Y-%m-%dT%H:%M:%SZ", gmtime(&t));
			fprintf(logfp,"%s [%s] %s\n",timebuf,username,s);
			fclose(logfp);
		} else {
			/* Failed to log to our file */
			syslog(prio,"[%s] %s",username,s);
		}
    }
    free(s);
