...
static struct option const long_options[] =
{
  ...
  {"skip-last-packet", no_argument, NULL, '0'},
...
main(int argc, char **argv)
{
	...
	while ((c = getopt_long (argc, argv, 
		"02+48abB:C:c:dfeEghHi:kL:l:m:M:NnOopRrqsSt:TUuvw:XYy",
		long_options, (int *) 0))!=EOF)
	{
        ...
        case '0': Skiplastpacket = TRUE; break;
...
wcsend (int argc, char *argp[])
{
	...
	if (zmodem_requested)
		saybibi ();
	else if (protocol != ZM_XMODEM && Skiplastpacket == FALSE) {
...
#ifdef ENABLE_SYSLOG
		if (enable_syslog)
			lsyslog(LOG_INFO, "Sent %ld bytes of %s/%s: %ld Bytes, %ld BPS",
...
wctxpn(struct zm_fileinfo *zi)
{
    ...
    else if (protocol == ZM_YMODEM && Filcnt == 1) {
		vstringf(_("Give your local XMODEM receive command now."));
		vstring("\r\n");
    }
    ...
    if (enable_syslog && protocol==ZM_YMODEM) {
        lsyslog(LOG_INFO,"Sending %ld bytes of %s",
                zi->bytes_total, zi->fname);
    }
#endif
	...
	*q++ = 0;
	...
	if (!Ascii && (input_f!=stdin) && *zi->fname && fstat(fileno(input_f), &f)!= -1)
		sprintf(p, "%lu %ld %o 0 %d %ld", (long) f.st_size, f.st_mtime,
...
"  -0, --skip-last-packet      do not send terminating null packet (YZ)\n"
